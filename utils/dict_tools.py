...
def dict_merge_deep(dict1, dict2):
    """
    Recursively merges dict1 and dict2 such that any values in dict2 override values in dict1
    :param dict1:
    :param dict2:
    ...
    for k,v in dict2.items():
        ...
        if k in dict1 and isinstance(dict1[k], dict) and isinstance(v, dict):
            outdict[k] = dict_merge(dict1[k], v)
        if k in dict1 and isinstance(dict1[k], list) and isinstance(v, list) and len(dict1[k]) == len(v):
            outdict[k] = [dict_merge(a, b) for a, b in zip(dict1[k], v)]
        # Otherwise just overwrite the key in dict1
        else:
            outdict[k] = dict2[k]
    return outdict
...
def dict_subtract(dict1, remove_keys):
    outdict = dict1.copy()
