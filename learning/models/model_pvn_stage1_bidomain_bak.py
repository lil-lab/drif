...
from data_io.model_io import load_pytorch_model, load_model_state_dict
...
class PVN_Stage1_Bidomain(nn.Module):
    ...
    def load_resources_before_execution(self, env_id):
        pass
    ...
    def forward(self, images, states, instructions, instr_lengths,
        ...
        S_W, SM_W = self.map_accumulator_w(F_W, M_W, add_mask=[True] * traj_len, reset_mask=reset_mask, show="acc" if IMG_DBG else "")
        S_W_poses = g_poses
    ...
    def get_dataset(self, data=None, envs=None, domain=None, dataset_names=None, dataset_prefix=None, eval=False, halfway_only=False):
        ...
        if not halfway_only:
            data_sources.append(aup.PROVIDER_CHUNK_LM_POS_DATA)
            ...
            data_sources.append(aup.PROVIDER_CHUNK_LANDMARKS_MENTIONED)
