...
from data_io.instructions import get_all_instructions, get_word_to_token_map, tokenize_instruction
...
from learning.modules.map_to_map.ratio_path_predictor_legacy import RatioPathPredictorLegacy
...
class PVN_Stage1_Bidomain(NavigationModelComponentBase):

    def __init__(self, run_name="", domain="sim"):
        super(PVN_Stage1_Bidomain, self).__init__(run_name, domain, model_name="pvnv2")
        ...
        self.include_map_boundaries = self.params["boundary_channels"] == 4
        if self.include_map_boundaries:
        ...
        is_legacy = self.params["is_legacy"]
        if is_legacy:
            self.path_predictor_lingunet = RatioPathPredictorLegacy(
    ...
    def forward(self, images, states, instructions, instr_lengths,
        ...
        if isinstance(instructions, list):
            ...
            instructions = torch.tensor([tokenize_instruction(i, word2token=self.word2token) for i in instructions], dtype=torch.long, device=images.device)
            instr_lengths = [len(instructions[0])]
        ...
        if instructions is not None:
            ...
            self.model_state.tensor_store.keep_inputs("sentence_embed", sent_embeddings)
        else:
        ...
        F_W, M_W = self.img_to_features_w(images, cam_poses, sent_embeddings, self.model_state.tensor_store, show="", halfway=halfway)

        if halfway == True and not halfway == "v2":
            return None, None

        self.model_state.tensor_store.keep_inputs("F_w", F_W)
        self.model_state.tensor_store.keep_inputs("M_w", M_W)
        ...
        S_W, SM_W = self.map_accumulator_w(self.model_state, F_W, M_W, add_mask=[True] * traj_len, reset_mask=reset_mask)
        S_W_poses = g_poses
        self.model_state.tensor_store.keep_inputs("SM_w", SM_W)
        ...
        # TODO: revise the need for this nonsense
        ...
        self.model_state.tensor_store.keep_inputs("S_W_select", S_W_select)
        self.prof.tick("batch_select")

        # Create a figure where the drone is drawn on the map
        if self.params["write_figures"]:
            self.model_state.tensor_store.keep_inputs("drone_poses", Variable(draw_drone_poses(cam_poses_select)))
        ...
        if self.use_aux["grounding_map"]:
            ...
            self.model_state.tensor_store.keep_inputs("R_W_select", R_W_select)
        ...
        # Concatenate map boundary mask with the 4 fences.
        if self.include_map_boundaries:
            self.boundary_mask = self.boundary_mask.to(RS_W_select)
            RS_W_select = torch.cat([RS_W_select, self.boundary_mask.repeat(RS_W_select.shape[0], 1, 1, 1)], 1)
        ...
        self.model_state.tensor_store.keep_inputs("RS_S_select", RS_S_select)
        self.model_state.tensor_store.keep_inputs("SM_S_select", SM_S_select)
        ...
        self.model_state.tensor_store.keep_inputs("RS_perturbed_select", RS_P_select)
        ...
        # ---------
        log_v_dist_p_select, v_dist_p_poses_select = self.path_predictor_lingunet(RS_P_select, sent_embeddings_pp, RS_P_poses_select, tensor_store=self.model_state.tensor_store)
        # ---------
        ...
        self.model_state.tensor_store.keep_inputs("log_v_dist_w_select", log_v_dist_w_select)
        ...
        self.model_state.tensor_store.keep_inputs("log_v_dist_s_select", log_v_dist_s_select)
        ...
        if self.get_iter() % 23 == 0 and not noshow:
            for i in range(S_W_select.shape[0]):
                Presenter().show_image(log_v_dist_p_select.softmax().visualize(), f"{self.domain}_v_dist_s_select_full", scale=4, waitkey=1)
    ...
    def unbatch(self, batch, halfway=False):
        ...
        if not halfway:
            ...
            self.model_state.tensor_store.keep_inputs("v_dist_s_ground_truth_select", v_dist_s_ground_truth_select)
            ...
            self.model_state.tensor_store.keep_inputs("lm_pos_fpv", lm_pos_fpv)
            self.model_state.tensor_store.keep_inputs("lm_pos_map", lm_pos_map_px)
            self.model_state.tensor_store.keep_inputs("lm_indices", lm_indices)
            self.model_state.tensor_store.keep_inputs("lm_mentioned", lm_mentioned)
            self.model_state.tensor_store.keep_inputs("lang_lm_mentioned", lang_lm_mentioned)
            ...
            self.model_state.tensor_store.keep_inputs("lm_pos_map_select", lm_pos_map_select)
            self.model_state.tensor_store.keep_inputs("lm_indices_select", lm_indices_select)
            self.model_state.tensor_store.keep_inputs("lm_mentioned_select", lm_mentioned_select)
            self.model_state.tensor_store.keep_inputs("goal_pos_map_select", goal_pos_map_select)
    ...
    def sup_loss_on_batch(self, batch, eval, halfway=False, grad_noise=False, disable_losses=[]):
        ...
        if halfway and not halfway == "v2":
            return None, self.model_state.tensor_store
        ...
        if not halfway:
            ...
            success_goal = self.goal_good_criterion(
                self.model_state.tensor_store.get_inputs_batch("log_v_dist_w_select", cat_not_stack=True),
                goal_pos
            )
    ...
    def get_dataset(self, data=None, envs=None, domain=None, dataset_names=None, dataset_prefix=None, eval=False, halfway_only=False):
        ...
        CHUNKS = True
        if not halfway_only:
            data_sources.append(aup.PROVIDER_CHUNK_LM_POS_DATA if CHUNKS else aup.PROVIDER_LM_POS_DATA)
            ...
            data_sources.append(aup.PROVIDER_CHUNK_LANDMARKS_MENTIONED if CHUNKS else aup.PROVIDER_LANDMARKS_MENTIONED)
