...
from transformations import img_to_as

import torch
from learning.inputs.common import empty_float_tensor, cuda_var
from learning.inputs.pose import Pose

#from learning.models.semantic_map.sm_params import \
#    IMAGE_X, IMAGE_Y, MAP_SIZE, MAP_WORLD_SIZE, WORLD_SIZE, DRONE_HEIGHT, USE_DEPTH, CAM_FOV, CAM_MAP_X, CAM_MAP_Y
...
class PinholeCameraProjection():
    ...
    def __init__(self,
        ...
        # In AirSim, the drone's starting position becomes the world origin.
        # The starting position is usually elevated, which means that the ground is not at elevation zero, but
        # at evelation -(start_height_meters)
        self.drone_start_height_meters = start_height_meters
    ...
    def world_point_to_image(self, cam_pos, cam_rot, point):
        ...
        cam_pos[2] += self.drone_start_height_meters
    ...
    def get_world_coord_grid(self):
        coord_grid = self.get_coord_grid(self.map_size, self.map_size, flip_x=False, flip_y=False, use_3d=True).reshape(-1, 4)
        coord_grid[:, 0:2] = img_to_as(coord_grid[:, 0:2], self.map_size, self.map_world_size)
        return coord_grid
    ...
    def get_projection_mapping(self, cam_pos, cam_rot, local_frame=False, range1=True):
        ...
        cam_pos = cam_pos.copy()
        cam_pos[2] = cam_pos[2] + self.drone_start_height_meters
        ...
        if not local_frame:
            ...
            pts_cam = pts_w
            pts_cam[:, 0:2] = pts_cam[:, 0:2] - self.map_world_size / 2
        ...
        pts_img = np.reshape(pts_img, [self.map_size, self.map_size, 2])

        return pts_img


class PinholeCameraProjectionModuleGlobal(CudaModule):
    ...
    def __init__(self,
        ...
        self.map_size = map_size
        self.projector = PinholeCameraProjection(map_size, world_in_map_size, world_size, img_w, img_h, cam_fov, False)
