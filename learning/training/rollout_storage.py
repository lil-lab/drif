...
class RolloutStorage(object):
    ...
    def initialize(self, rollouts, intrinsic_reward_only=False):
        for rollout in rollouts:
            for sample in rollout:
                ...
                self.actions[self.step] = torch.from_numpy(sample["action"])
                self.action_log_probs[self.step] = sample["action_logprob"]

                # Predicted value for observation at timestep t
                self.value_preds[self.step] = sample["value_pred"]
    ...
    def initialize_c(self, rollouts, intrinsic_reward_only=False):
        for rollout in rollouts:
            for sample in rollout:
                # Observation at start of timestep t
                self.obs[self.step] = sample["policy_input"].to("cpu").clone().detach()
                self.obsb[self.step] = sample["policy_input_b"].to("cpu").clone().detach()
                # Action taken at timestep t
