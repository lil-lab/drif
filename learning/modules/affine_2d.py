...
class Affine2D(nn.Module):
    ...
    def forward(self, image, affine_mat, out_size=None):
        ...
        grid = F.affine_grid(affines_pytorch[:, [0,1], :], torch.Size((batch_size, 1, out_size[0], out_size[1])), align_corners=False).float()

        self.prof.tick("affine_grid")

        # Rotate the input image
        rot_img = F.grid_sample(image, grid, padding_mode="zeros", align_corners=False)
