...
class LeakyIntegratorGlobalMap(MapTransformerBase):
    ...
    def __init__(self, source_map_size, world_in_map_size, lamda=0.2):
        super(LeakyIntegratorGlobalMap, self).__init__(source_map_size, world_in_map_size)
    ...
    def dbg_write_extra(self, map, pose):
        if DebugWriter().should_write():
            ...
            if pose is not None:
                ...
                map_global = map
            DebugWriter().write_img(map_global[0], "gif_overlaid", args={"world_size": self.world_size, "name": "sm"})
    ...
    def forward(self, images_w, coverages_w, add_mask=None, reset_mask=None, show=False):
        ...
        for i in range(batch_size):
            ...
            if show != "":
                Presenter().show_image(new_map_w.data[0, 0:3], show, torch=True, scale=8, waitkey=50)
