...
class PVN_Stage2_RLBase(nn.Module):
    def __init__(self, map_channels=1, map_struct_channels=1, map_size=32, crop_size=16, h1=8, structure_h1=8, h2=128, obs_dim=16, name="base"):
        ...
        linear_in = int(((crpsz / 2) ** 2) * h1) + (8*8*structure_h1) + 2*obs_dim # map channels + coverage channels + observability encoding channels
    ...
    def forward(self, maps_r, map_structure_r):
        ...
        if maps_r_cropped.shape[2] == 32 and maps_r_cropped.shape[3] == 32:
            maps_r_cropped = F.avg_pool2d(maps_r_cropped, 2, stride=2)
        ...
        c = self.covnorm1(c)
        #c.register_hook(partial(self.backward_hook, "conv1c_out_norm"))

        if False:
            Presenter().show_image(uncov_r_pooled, "map_struct_pooled", scale=8, waitkey=1)

        comb_map = torch.cat([x,c], dim=1)
        batch_size = x.shape[0]
        lin_in = comb_map.view(batch_size, -1)
        if obs_context_vec.std().detach().item() > 5.0:
            print(f"Lin: {lin_in.mean()} {lin_in.std()}")
            print(f"Obs: {obs_context_vec.mean()} {obs_context_vec.std()}")
        ...
        #x.register_hook(partial(self.backward_hook, "lin2_out"))
