...
class MapTransformerBase(nn.Module):
    ...
    def set_map(self, model_state, map, pose):
        model_state.latest_maps = map
        model_state.latest_map_poses = pose

    def set_maps(self, model_state, maps, poses):
        model_state.latest_maps = maps
        model_state.latest_map_poses = poses

    def add_maps(self, model_state, maps, poses):
        model_state.latest_maps = torch.cat([self.latest_maps, maps], dim=0)
        model_state.latest_map_poses = self.latest_map_poses + poses

    def forward(self, model_state, maps, map_poses, new_map_poses):
