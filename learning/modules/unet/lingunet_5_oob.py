...
class Lingunet5OOB(torch.nn.Module):
    def __init__(self, params):
        ...
        print("Upscale conv: ", self.p.upscale_conv)
        print("Double up: ", self.p.double_up)
        print("Double down: ", self.p.double_down)
    ...
    def forward(self, input, embedding, tensor_store=None):
        ...
        if tensor_store is not None and False:
        ...
        self.prof.tick("encode")

        if embedding is not None and not self.p.noling:
            ...
            assert embedding.shape[0] == 1, "Instruction sequences are no longer supported"
            emb_idx = 0
        ...
        self.prof.tick("decode")
        self.prof.loop()
        self.prof.print_stats(every_n_times=1)
