...
class Presenter:
    ...
    def __init__(self):
        ...
        self.headless = params["Environment"].get("headless", False) if params else False
    ...
    def _load_drone_img(self):
        ...
        drone_path = os.path.join(get_sprites_dir(), "drone_img_r_triangle.png")# "drone_img_u.png")
    ...
    def draw_trajectory(self, image, positions, world_size_m, color="c", convert=False):
        h = image.shape[0]
        w = image.shape[1]

        x_targets_o, y_targets_o = list(zip(*positions))
        if convert:
            y_targets = [h - x for x in x_targets_o]
            x_targets = [y for y in y_targets_o]
        else:
            x_targets, y_tagets = x_targets_o, y_targets_o
    ...
    def _transform_img_to_pose(self, image_out, img_to_transform, pose, img_scale, convert=False):
        ...
        if convert:
            x = pos[0]
            y = pos[1]
            pos = np.asarray([img_size_px - y, x])
        trans_transform = get_affine_trans_2d(pos)
    ...
    def draw_drone(self, image, pose_m, world_size_m, convert=False):
        ...
        drone_img_t, drone_img_mask = self._transform_img_to_pose(image, drone_img, pose_px, DRONE_SIZE_FRACTIONAL, convert=convert)
    ...
    def scale_image(self, image, scale, interpolation="nearest"):
        ...
        image_o = cv2.resize(image, dsize=tuple(reversed([int(x) for x in np.asarray(image.shape)[:2]*scale])), interpolation=interpolation)
        return image_o
    ...
    def prep_image(self, image, scale=(1.0, 1.0), no_norm=False, interpolation=cv2.INTER_NEAREST):
        import cv2
        ...
        if scale != 1.0:
            ...
            image = cv2.resize(image, (int(scale[0]*width), int(scale[1]*height)), interpolation=interpolation)
    ...
    def overlaid_image(self, background, overlay, gray_bg=False, channel=None, strength=0.4, bg_strength=None, interpolation=cv2.INTER_NEAREST):
        background = self.prep_image(background)
        overlay_t = self.prep_image(overlay)

        if bg_strength is None:
            bg_strength = 1 - strength

        scale_x = int(background.shape[0] / overlay_t.shape[0])
        scale_y = int(background.shape[1] / overlay_t.shape[1])

        overlay = self.prep_image(overlay, scale=(scale_x, scale_y), interpolation=interpolation)
        out = np.zeros_like(background)
        # Place the background
        if gray_bg:
            out[:, :, :] = np.expand_dims(np.mean(background, axis=2) * bg_strength, 2)
        else:
            out[:, :, :] = background * bg_strength
    ...
    def save_image(self, image, name="live", torch=False, draw_point=None, scale=1.0, folder=""):
        ...
        imageio.imsave(folder + "/" + name + ".png", image)
