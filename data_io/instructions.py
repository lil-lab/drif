...
def get_instruction_segment(env_id, set_idx, seg_idx, full=True, all_instr=None):
...
def get_segs_available_for_env(env_id, set_idx, full=False, all_instr=None):
    if all_instr is None:
        train_instr, dev_instr, test_instr, corpus = get_all_instructions(full=full)
        all_instr = {**train_instr, **dev_instr, **test_instr}
...
def get_restricted_env_id_lists(prune_ambiguous=False, full=False):
