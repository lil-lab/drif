...
def spawn_headless_worker(instance_id, port):
    ...
    print(f" . ' . ' . ' . ' . ' . ' . ' . ' . ' . ' . ' . ' . ' . ' . ' . ' . ' . ' . ' ")
    config_path = paths.get_sim_config_path_for_instance(instance_id)
    command = paths.get_sim_executable_path() + " WorkerID " + str(
        instance_id) + " ApiServerPort " + str(port) + " ConfigPath " + str(config_path)
...
def spawn_worker(instance_id, port):
    ...
    #command += " ConfigPath " + str(config_path)
    command += " WorkerID " + str(instance_id) + " ApiServerPort " + str(port) + " ConfigPath " + str(config_path)
    print("Executing: ", command)
    subprocess.Popen(command, env=os.environ, shell=True)
...
def startAirSim(controller, instance_id, port):
    try:
        ...
        try:
            ...
            print("FAILED AGAIN! Exception: ", str(e))
...
class DroneController(DroneControllerBase):
    def __init__(self, instance=0, flight_height=100.0):
        ...
        self.empty_env = P.get_current_parameters()["Setup"]["empty_envs"]
        self.empty_conf = self._make_empty_conf()
