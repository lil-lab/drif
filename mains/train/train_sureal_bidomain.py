...
def train_supervised_worker(rl_process_conn):
    ...
    if start_sup_epoch == 0:
        save_pytorch_model(model_real, epoch_sup_filename(run_name, -1, model="stage1", domain="real"))
        save_pytorch_model(model_sim, epoch_sup_filename(run_name, -1, model="stage1", domain="sim"))
        save_pytorch_model(model_critic, epoch_sup_filename(run_name, -1, model="stage1", domain="critic"))
    if start_sup_epoch > 0:
