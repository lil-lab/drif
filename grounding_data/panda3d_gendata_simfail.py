...
rollout_dir = "/media/clic/shelf_space/cage_workspaces/unreal_config_nl_cage_rss2020/data/simulator_empty"
data_out_dir = "/media/clic/BigStore/grounding_data_both_sim/raw"
...
class MyApp(ShowBase):
    def __init__(self):
        ...
        props.setSize(128*6, 96*6)
        self.win.requestProperties(props)

        if DATA_SPLIT == "test":
            self.num_scenes = 10;
        elif DATA_SPLIT == "train":
            self.num_scenes = 4000
    ...
    def run(self):
        ...
        for scene_idx in range(self.num_scenes):
            # TODO: Generate random scene, including selecting the "active" object in this scene
            self.newScene()
            ...
            for traj_idx, rollout in enumerate(picked_rollouts):
                ...
                for t, sample in enumerate(rollout):
                    ...
                    #cv2.imshow("bgimg", background_image)
                    #self.setBackgroundFromNumpy(background_image)
                    ...
                    self.compute_and_save_frame_geometry(f"{scene_idx}_{traj_idx}_{t}--geometry")
                    ...
                    # Switch to ground plane only mode
                    #self.setMode("background")
                    #self.do_render()
                    #self.save_screenshot(f"{scene_idx}_{traj_idx}_{t}--background_3d")
    ...
    def newScene(self):
        ...
        for y_pos, x_pos, radius, rpy, landmarkName in zip(
            ...
            #posnow = landmark.getPos()
            #wlhnow = self.obj_loader.getWLH(landmark)
            #boundsnow = landmark.getTightBounds(self.render)
    ...
    def compute_and_save_frame_geometry(self, name):
        ...
        for obj_id, object in zip(self.landmark_ids, self.landmarks):
            ...
            all_points_in = True
            for ptin in points_in:
                all_points_in = all_points_in and ptin
            if all_points_in:
